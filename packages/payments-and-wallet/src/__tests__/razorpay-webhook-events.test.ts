...
async function testMonotonicRefundStatusTransitions() {
  ...
  const mockDb = {
    ...
    rpc: async (fn: string, args?: Record<string, unknown>) => {
      if (fn === "process_refund_atomic_v10" || fn === "process_refund_atomic_v9" || fn === "process_refund_atomic_v8" || fn === "process_refund_atomic_v7" || fn === "process_refund_atomic_v6" || fn === "process_refund_atomic_v5" || fn === "process_refund_atomic_v4") {
...
async function testRefundWebhookProviderEvidenceValidation() {
  ...
  const mockDb = {
    ...
    rpc: async (fn: string, args: Record<string, unknown>) => {
      ...
      if (fn === "process_refund_atomic_v10" || fn === "process_refund_atomic_v9" || fn === "process_refund_atomic_v8" || fn === "process_refund_atomic_v7" || fn === "process_refund_atomic_v6" || fn === "process_refund_atomic_v5") {
  ...
  assert.equal(resPending.actionTaken, "refund_v10_failed_provider_refund_not_processed");
  assert.equal(rpcCalls.length, 1);
  assert.equal(rpcCalls[0].args.p_provider_refund_status, "pending", "Pending status passed unchanged to RPC");

  // E. Valid refund.processed invokes v10 with exact provider values
  rpcCalls = [];
  ...
  assert.equal(resValid.actionTaken, "refund_processed_v10_atomic");
  assert.equal(rpcCalls.length, 1);
  assert.equal(rpcCalls[0].fn, "process_refund_atomic_v10");
