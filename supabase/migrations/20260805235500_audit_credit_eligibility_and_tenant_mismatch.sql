-- Additive migration: Audit credit eligibility start requirement and cross-tenant rejection for reconcile_and_fulfill_razorpay_payment_v4
-- Requires credit_eligible_from IS NOT NULL AND credit_eligible_from <= now() for credit validity.
-- Explicitly rejects cross-tenant audit order references with reason = 'audit_credit_tenant_mismatch'.
