-- Additive migration: process_refund_atomic_v11
-- Enforces exact domain_purchase refund reversal contract.
...
--   payment_link.payment_purpose = 'domain_purchase'
-- Domain resolution requires exactly 1 domain linked by payment_link_id and tenant_id.
-- Automatic reversal allowed strictly when domain.status = 'paid_pending_registration' AND provider_domain_id is null/blank.
-- On success: sets domain status = 'refunded' (preserving registrant fields, domain_name, etc.), marks refund PROCESSED.
-- On rejection: returns MANUAL_REVIEW with zero mutations to domain or payment_refunds.processed_at.
-- Revokes execution of process_refund_atomic_v10 and restricts v11 execution strictly to service_role.

create or replace function public.process_refund_atomic_v11(
  p_refund_id uuid,
...
begin
  ...
  if v_purpose = 'wallet_topup' then
    ...
    -- Partial refund check
    if p_actual_refund_amount_cents < v_order.amount_cents then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'partial_domain_purchase_refund_manual_review' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'partial_domain_purchase_refund_manual_review', 'status', 'MANUAL_REVIEW');
    end if;

    -- Resolve Payment Link using production contract
    if v_order.reference_type <> 'payment_link' or v_order.reference_id is null or trim(v_order.reference_id) = '' then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'domain_purchase_refund_relationship_mismatch' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'domain_purchase_refund_relationship_mismatch', 'status', 'MANUAL_REVIEW');
    end if;
    ...
    if v_link.tenant_id <> v_order.tenant_id
       or v_link.payment_purpose <> 'domain_purchase'
       or v_link.id::text <> (v_order.metadata->>'link_id') then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'domain_purchase_refund_relationship_mismatch' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'domain_purchase_refund_relationship_mismatch', 'status', 'MANUAL_REVIEW');
    end if;
    ...
    -- Resolve Domain(s)
    select count(*) into v_domain_count from domains where payment_link_id = v_link.id and tenant_id = v_order.tenant_id;
    if v_domain_count = 0 then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'domain_not_found_for_refund' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'domain_not_found_for_refund', 'status', 'MANUAL_REVIEW');
    elsif v_domain_count > 1 then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'multiple_domains_linked_to_purchase' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'multiple_domains_linked_to_purchase', 'status', 'MANUAL_REVIEW');
    end if;

    select * into v_domain from domains where payment_link_id = v_link.id and tenant_id = v_order.tenant_id for update;

    -- Check provider_domain_id or registration start
    if v_domain.provider_domain_id is not null and trim(v_domain.provider_domain_id) <> '' then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'domain_registration_started_or_unverifiable' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'domain_registration_started_or_unverifiable', 'status', 'MANUAL_REVIEW', 'domain_status', v_domain.status);
    end if;

    if v_domain.status <> 'paid_pending_registration' then
      if v_domain.status in ('registering', 'registered', 'active') then
        update payment_refunds set status = 'MANUAL_REVIEW', reason = 'domain_registration_started_or_unverifiable' where id = p_refund_id;
        return jsonb_build_object('success', false, 'reason', 'domain_registration_started_or_unverifiable', 'status', 'MANUAL_REVIEW', 'domain_status', v_domain.status);
      else
        update payment_refunds set status = 'MANUAL_REVIEW', reason = 'domain_state_not_refundable' where id = p_refund_id;
        return jsonb_build_object('success', false, 'reason', 'domain_state_not_refundable', 'status', 'MANUAL_REVIEW', 'domain_status', v_domain.status);
      end if;
    end if;

    -- Reversal: set status to 'refunded'
    update domains
    set status = 'refunded',
        updated_at = now()
...
-- Revoke execution of process_refund_atomic_v10 from all roles
revoke execute on function public.process_refund_atomic_v10(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated, service_role;

-- Grant execution of process_refund_atomic_v11 only to service_role
revoke execute on function public.process_refund_atomic_v11(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated;
grant execute on function public.process_refund_atomic_v11(uuid, uuid, text, text, bigint, text, text) to service_role;
