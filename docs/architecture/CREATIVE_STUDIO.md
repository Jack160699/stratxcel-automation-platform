...
|------------|------------|----------------|
| Image generation | `ImageMediaRuntime` uses Gemini image models with an OpenAI image fallback, persists candidates to canonical tenant storage, and requires human selection | Explicit setup/failure state with no candidate or fake URI |
...
The customer workspace at `/app/content/studio` and its authenticated `/api/platform/image-generations` routes use the same server-side runtime as Social Copilot and Workforce. Brand Brain context is reduced to an approved provider-boundary allowlist. Generation jobs, references, candidates, critique metadata, revisions, selection, errors, usage state, and provenance are durable tenant-scoped records. Generated bytes are copied into the private canonical media bucket and linked through `social_media_assets`; signed URLs are presentation-only.

`image_generation_jobs.idempotency_key` prevents duplicate paid jobs. Ambiguous provider timeouts are not automatically sent to a fallback. A configured provider without writable storage, an active subscription, or a working usage ledger remains non-operational.
