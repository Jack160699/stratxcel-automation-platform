...
## Update 2026-08-10 — the per-mission tool bridge is now built and live-verified

With every built-in toolset disabled, Hermes has zero local tools of its own — but it can still connect to **external** MCP servers via `mcp_servers` in `config.yaml`, a real, documented (if under-documented — see below) feature using the official Python `mcp` SDK's `streamablehttp_client`. StratExcel now runs its own MCP server as an extension of `apps/hermes-gateway`, and Hermes is configured to connect to it. This closes the gap described above — the statement "the tool bridge is not yet functional" is retired as of this update, replaced by the real architecture:

```
Hermes Agent
  -> private Streamable HTTP MCP (mcp_servers.stratxcel in Hermes's config.yaml)
  -> apps/hermes-gateway /mcp
  -> static transport auth (STRATXCEL_MCP_BRIDGE_SECRET) + mission capability validation
  -> existing invokeTool() (packages/hermes/src/tools/schemas.ts validates first)
  -> tenant/mission isolation, allowedTools, kill switches, audit, correlation IDs (unchanged)
  -> StratExcel services
```

**The security model, precisely:** Hermes globally discovers and can *see* all 10 StratExcel-callable tools for every mission on this installation — that has not changed and cannot be changed from Hermes's side (see above). What changed is that every tool *execution* now requires a real, per-mission capability (the same `issueMissionToken`/`verifyMissionToken` this repo already used for `POST /tools/<name>`) supplied as a `missionCapability` tool argument. Seeing a tool is not the same as being authorized to use it — `authorizeMcpToolCall()` in `mcp-server.ts` independently re-verifies the token, its expiry, and `allowedTools` on every single call, regardless of what the model attempts. Two layers of auth exist, checked in order: (A) a static `STRATXCEL_MCP_BRIDGE_SECRET` bearer token, at the HTTP transport level, proving the caller is genuinely this Hermes installation — unrelated to any mission; (B) the mission capability token, per tool call, proving *this specific call* is authorized for *this specific mission's* tenant and tool set. `submit_publish_request`/`create_website_change_request` have no schema entry in `packages/hermes/src/tools/schemas.ts` and are therefore never registered as MCP tools at all — StratExcel remains the sole authority for publishing and deployment.

**Real, live-verified evidence (2026-08-10), not simulated:**
- `GET /v1/toolsets` on Hermes: all 27 built-in toolsets still `enabled: false` after enabling the bridge — the fix above was not regressed.
- A genuine `initialize` MCP handshake (Streamable HTTP, protocol `2025-06-18`) against `apps/hermes-gateway`'s `/mcp` succeeded only with the correct `STRATXCEL_MCP_BRIDGE_SECRET`; no/wrong secret → `401`.
- `tools/list` returned exactly the 10 callable tools, correctly annotated `readOnlyHint`/`destructiveHint`, and neither controlled tool.
- A synthetic mission token scoped to exactly `get_brand_context`, driven through a **real** `POST /v1/runs` on the live Hermes instance (free `nvidia/nemotron-3-super-120b-a12b:free` model, real inference, not mocked): the model genuinely invoked `mcp__stratxcel__get_brand_context` via Hermes's own tool-calling — confirmed both by the run's final output and by a real `hermes.tool_call.get_brand_context` row appearing in `audit_events` with a real correlation ID.
- The **same token**, in a second real run, asked to call `mcp__stratxcel__create_crm_lead` instead: the bridge rejected it server-side ("not in this mission's allowed tool set") and the model correctly reported the denial — the mandatory release-blocking proof that a real non-allowlisted call fails even when Hermes itself relays it.
- **One real deployment bug found and fixed along the way:** `config.yaml`'s documented `${VAR}` environment-variable interpolation does **not** apply inside `mcp_servers.*.headers` values in this Hermes version — confirmed via `hermes config get mcp_servers`, which showed the literal, unsubstituted string `${STRATXCEL_MCP_BRIDGE_SECRET}` being sent as the bearer token, causing a real `401` and Hermes "parking" the connection (it does not auto-retry after an auth failure — a config or restart is required to reconnect). Fixed by writing the resolved secret value directly into `config.yaml` instead of relying on interpolation for this specific field.
