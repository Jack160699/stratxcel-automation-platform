...
export function buildVisualDirectorBrief(input: {
  ...
  const archetypeSpaceNote: Record<CreativeTreatment["layoutArchetype"], string> = {
    ...
    FEATURE_POSTER: `This creative uses FEATURE_POSTER: after generation, this photo will be cropped into a single bounded, rounded-corner panel occupying roughly one-third of the canvas width on a plain light/white poster background -- it is NOT the full canvas and will NOT have any text composited directly onto it. Compose a tight, well-balanced single-subject shot (a product, a person, a service moment) that still reads clearly when cropped to a narrower vertical panel -- avoid a wide establishing shot or a composition that depends on content near the frame's edges.`,
  };
