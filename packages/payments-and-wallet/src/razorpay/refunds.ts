...
/**
 * Marks a refund processed and routes product reversal atomically via process_refund_atomic_v6 RPC.
 * Production code MUST use the atomic v6 RPC and pass authentic provider evidence.
 */
export async function markRefundProcessed(
  ...
  const { data: rpcData, error: rpcErr } = await supabase.rpc("process_refund_atomic_v6", {
  ...
  if (rpcErr) {
    throw new Error(`process_refund_atomic_v6 RPC error: ${rpcErr.message}`);
  }

  if (!rpcData || typeof rpcData !== "object") {
    throw new Error("process_refund_atomic_v6 RPC returned invalid response");
  }
  ...
  if (!res.success) {
    ...
    throw new Error(`process_refund_atomic_v6 RPC failed: ${res.reason}`);
  }
