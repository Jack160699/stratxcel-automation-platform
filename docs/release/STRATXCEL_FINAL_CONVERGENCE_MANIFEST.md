...
**Update — reconciled for real, one at a time, each verified against live schema before and after (not filename-guessed):**

| Migration | Real live state found | Action |
|---|---|---|
| `image_generation_creative_treatment` | `creative_treatment` column already present, `text_overlay_applied` missing | **Applied** (`add column if not exists` — safe, additive-only) |
| `social_autopilot_archetype_tier_quotas` | Attempted; **Postgres itself rejected it** (`23514 check constraint ... is violated by some row`) — its 8-metric constraint list omits `image_generation_attempts_monthly`/`copilot_monthly_uses`, which real live rows already use. Checked further: `social_autopilot_automated_monthly`/`manual_monthly` entitlement rows **already exist live** for real subscriptions. | **ALREADY_PRESENT — not reapplied.** The goal state was already achieved via some other path; forcing this file's narrower constraint would have been a real regression, correctly blocked by the database's own constraint, not by me. |
| `social_autopilot_visual_archetype_preferences` | Table (`social_autopilot_visual_preferences`) already existed live, byte-for-byte matching schema | **Applied anyway** (fully idempotent `if not exists`/`drop policy if exists` — closes the migration-history bookkeeping gap, zero functional change) |
| `commercial_model_v3_rebuild` | Constraint already matched exactly; `website_custom_quotes` table missing | **Applied** (constraint no-op confirmed safe; table + RLS created) |
| `razorpay_reconciliation_v3_catalog_support` | Neither function updated yet | **Applied** — `create or replace function`, not a fragile pattern-patch, so safe regardless of prior state. This is the one migration in this set with real, documented live-money stakes (its own header: a real webhook handler would have captured a real customer's payment and never activated their subscription) — read in full (721 lines) before applying, not skimmed. |
| `audit_monthly_allowance` | `audit_requests` not yet in the constraint | **Applied** (pure widening on top of the now-current state, verified safe) |
| `social_autopilot_weekly_campaigns` | Table already existed live, byte-for-byte matching schema | **Applied anyway** (same bookkeeping-gap reasoning as above) |
| `festival_calendar_rules` | Real, differently-named `festival_calendar_rules_production_fix` already live with the expected 13 rows; this file's own `insert` has no `on conflict` guard | **OBSOLETE — not applied.** Re-running risks an unnecessary insert error for a need already met. |
| `ecommerce_engine_schema` | Target tables (`ecommerce_categories` etc.) don't exist; **zero real application code anywhere references them** (`git grep` across the whole branch) | **Not applied — no current product need.** Speculative schema with no consumer; revisit if/when an e-commerce feature actually ships. |

A related `function_search_path_mutable` advisory the first of these introduced (`social_autopilot_array_has_duplicates`) was fixed immediately after (`set search_path = public`) — verified via `get_advisors` before and after, zero other new findings from any of these seven applies.

Full re-verification after all seven applies: `test:subscriptions-billing`, `test:social-package-autopilot`, `test:image-generation` all still pass; `get_advisors` (security) shows no new findings beyond the one fixed.
...
2. **`test:payment-concurrency` — not a code defect, and not substitutable.** Read the full script (`scripts/test-payment-concurrency.ts`): it deliberately requires a direct raw Postgres connection (`CONCURRENCY_TEST_DATABASE_URL`) to a genuinely isolated database — localhost or a real Supabase preview branch — plus an explicit `ALLOW_PAYMENT_CONCURRENCY_TESTS=true` opt-in, and it hard-blocks the real production project ref by name. It exists to test real two-connection race conditions against Postgres's own row-locking/constraint behavior, which by its nature cannot be faked with a mock — that's the entire point of the test. **`WAITING_EXTERNAL`** — needs the account owner to provision an isolated test database (local Postgres or a Supabase branch); not something to weaken, delete, or fabricate around.
