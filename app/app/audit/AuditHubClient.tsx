...
export function AuditHubClient({ initialData }: { initialData: AuditHubData }) {
  ...
  const [generation, setGeneration] = useState<AuditGeneration | null>(
    initialData.generation ? (initialData.generation as unknown as AuditGeneration) : null
  );
  const [error, setError] = useState<string | null>(null);
  const [starting, setStarting] = useState(false);
  const autoStartAttempted = useRef(false);

  const [brandBrain, setBrandBrain] = useState<Record<string, unknown> | null>(
    initialData.brandBrain
  );
  const [googleBusinessConnectionState, setGoogleBusinessConnectionState] = useState<string | null>(
    initialData.googleBusinessConnectionState
  );
  ...
  const [waMasked, setWaMasked] = useState<string | null>(initialData.whatsappDestination?.masked ?? null);
