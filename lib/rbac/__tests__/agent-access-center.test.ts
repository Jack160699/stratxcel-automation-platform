...
function run() {
  ...
  const copilot = read("lib","agent-core","copilot-actions.ts");
  const migration = read("supabase","migrations","20260809024101_shared_agent_brain.sql");
  ...
  assert.match(copilot, /parsed\.kind === "reset"/, "Web Copilot must handle RESET deterministically before the LLM");
  assert.match(copilot, /handleReset\(supabase, principal\)/, "Web Copilot RESET must use the shared scoped session reset");
