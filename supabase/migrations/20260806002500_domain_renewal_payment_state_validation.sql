-- Additive migration: Domain renewal payment state validation for reconcile_and_fulfill_razorpay_payment_v4
-- Adds 'paid_pending_renewal' to domains status constraint.
-- Restricts domain_renewal payments to renewal-eligible states ('active', 'registered', 'expired').
-- Rejects pending_payment, paid_pending_registration, registering, paid_pending_renewal, refunded, cancelled, transferred_out with reason = 'domain_renewal_state_not_payable'.
-- Successful domain renewal sets domain status to 'paid_pending_renewal' without extending expires_at prior to registrar confirmation.

-- 1. Ensure domains status constraint includes 'paid_pending_renewal'
