-- Additive migration: Audit-fee payment state validation for reconcile_and_fulfill_razorpay_payment_v4
-- Ensures audit_orders status constraint includes terminal states ('refunded', 'cancelled').
-- Restricts audit fee payments to payable state ('pending_payment').
-- Rejects paid, completed, refunded, cancelled audit orders with reason = 'audit_order_state_not_payable'.

-- 1. Ensure audit_orders status constraint allows terminal states
alter table audit_orders drop constraint if exists audit_orders_status_check;
alter table audit_orders add constraint audit_orders_status_check
  check (status in ('pending_payment', 'paid', 'in_review', 'completed', 'refunded', 'cancelled'));

-- 2. Update reconcile_and_fulfill_razorpay_payment_v4
...
begin
  ...
  -- ============================================================
  -- 5. PRE-MUTATION VALIDATIONS (Product resolution, Plan Tier & Audit/Subscription State Check)
