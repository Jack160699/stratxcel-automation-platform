-- Additive migration: process_refund_atomic_v6
-- Enforces fail-closed routing for unimplemented or unknown refund purposes (audit_fee, domain_renewal, null, or unrecognized).
-- Unimplemented purposes set status = 'MANUAL_REVIEW', leave processed_at = null, create no wallet/product/entitlement mutations, and return success = false.
-- Revokes execution of process_refund_atomic_v5 and restricts v6 execution strictly to service_role.

create or replace function public.process_refund_atomic_v6(
  p_refund_id uuid,
...
begin
  ...
  -- Check for unimplemented / unsupported refund purposes BEFORE any mutation or setting status = 'PROCESSED'
  ...
  -- 14. PRODUCT REVERSAL MUTATION BY PURPOSE (Only for automatically supported purposes: wallet_topup, subscription_payment, continuation_pack, domain_purchase)
  ...
  -- 15. Mark refund PROCESSED (Only reached for supported purposes with successful automatic reversal)
...
-- Revoke execution of process_refund_atomic_v5 from all roles
revoke execute on function public.process_refund_atomic_v5(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated, service_role;

-- Grant execution of process_refund_atomic_v6 only to service_role
revoke execute on function public.process_refund_atomic_v6(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated;
grant execute on function public.process_refund_atomic_v6(uuid, uuid, text, text, bigint, text, text) to service_role;
