...
export function WhatsAppSocialReview({ previews: initialPreviews, handoffToken, approveToken, editToken, cancelToken }: { previews: PublishActionPreview[]; handoffToken: string; approveToken: string; editToken: string; cancelToken: string }) {
  ...
  return <main className="mx-auto max-w-3xl p-4 sm:p-8">
    ...
    <h1 className="mt-2 text-2xl font-semibold">Ready for review</h1>
    <p className="mt-1 text-sm text-slate-500">{previews.length} prepared platform post{previews.length === 1 ? "" : "s"}</p>
    <div className="mt-6 space-y-3">{previews.map((preview) => <article key={preview.actionId} className="rounded-xl border bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3"><div><strong>{preview.platformLabel}</strong><small className="ml-2 text-emerald-700">Recommended</small></div><button className="rounded-lg border px-3 py-1.5 text-sm" onClick={() => setOpened(preview)}>Preview</button></div>
      ...
      <p className="mt-3 line-clamp-3 whitespace-pre-wrap text-sm">{preview.caption}</p>
    </article>)}</div>
