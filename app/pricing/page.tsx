    ...
    <PublicPageShell beforeMain={<PricingViewTracker />}>
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h1 className="mt-2 font-sx-sans text-3xl font-extrabold tracking-tight text-sx-text sm:text-5xl">
      ...
      <section className="border-t border-sx-border bg-sx-surface-2">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-xl font-bold">Which plan fits your business?</h2>
          </div>
          <PricingPlanGuide />
        </div>
      </section>
      ...
      {ORDER.map((pillarId) => {
        ...
        return (
          <section
            key={pillarId}
            className={`border-t border-sx-border ${pillarId === "platform" ? "bg-sx-surface-2" : ""}`}
          >
            <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
              ...
              {auditOnly ? (
                <div className="mx-auto mt-10 max-w-4xl rounded-sx-lg border border-sx-accent/30 bg-sx-surface-1 p-8 text-center shadow-[var(--sx-public-shadow-md)]">
                  <h3 className="text-2xl font-bold">
                    {tiers[0].name} — {tiers[0].price}
                  </h3>
                  <p className="mt-2 text-sm text-sx-text-muted">{tiers[0].pitch}</p>
                  <p className="mt-2 text-xs text-sx-text-muted">
                    <strong>Best for:</strong> {tiers[0].whoItsFor}
                  </p>
                  <TrackedCtaLink
                    href={tiers[0].href}
                    event="start_audit"
                    surface="pricing_audit"
                    plan="audit"
                    className="mt-6 inline-block rounded-sx-sm bg-sx-accent px-8 py-3 text-xs font-bold text-sx-accent-on"
                  >
                    {tiers[0].cta} →
                  </TrackedCtaLink>
                </div>
                ...
                <div
                  className={`mt-10 grid gap-8 ${tiers.length >= 3 ? "sm:grid-cols-2 lg:grid-cols-3" : "mx-auto max-w-4xl sm:grid-cols-2"}`}
                >
                  {tiers.map((t) => (
                    <Card
                      key={t.id}
                      variant={t.popular ? "elevated" : "panel"}
                      className={`flex h-full flex-col border p-8 ${t.popular ? "border-sx-accent/60 ring-2 ring-sx-accent/30" : ""}`}
                    >
                      ...
                      <p className="mt-3 rounded-sx-sm bg-sx-surface-2 px-3 py-2 text-xs text-sx-text-muted">
                        <strong>Best for:</strong> {t.whoItsFor}
                      </p>
                      ...
                      {t.upgradePath ? (
                        <p className="mt-4 text-[11px] text-sx-text-subtle">
                          <strong>Upgrade:</strong> {t.upgradePath}
                        </p>
                      ) : null}
                      <p className="mt-4 border-t border-sx-border pt-4 text-[11px] italic text-sx-text-subtle">
                        {t.note}
                      </p>
                      <TrackedCtaLink
                        href={t.href}
                        event={t.href.startsWith("/signup") ? "signup_intent" : undefined}
                        surface="pricing_tier"
                        plan={t.planKey}
                        className={`mt-6 block rounded-sx-sm py-3 text-center text-xs font-bold ${t.popular ? "bg-sx-accent text-sx-accent-on" : "border border-sx-border-strong"}`}
                      >
                        {t.cta}
                      </TrackedCtaLink>
                    </Card>
      ...
      <ConversionFaqSection
        ...
        subtitle="Transparent answers — no invented refund policies."
        items={PRICING_OBJECTIONS}
        className="border-t border-sx-border bg-sx-surface-1 py-16"
      />
      <CommercialTrustSection />
      <ConversionCtaBand surface="pricing_footer" showDemo />
      ...
      <section className="mx-auto max-w-2xl px-4 py-10 text-center">
        ...
        <div className="mt-6 flex flex-col justify-center gap-4 sm:flex-row">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-sx-sm bg-sx-accent px-6 py-3 text-xs font-bold text-sx-accent-on"
          >
            Chat on WhatsApp
          </a>
          <Link
            href="/contact?intent=custom"
            className="rounded-sx-sm border border-sx-border-strong px-6 py-3 text-xs font-semibold"
          >
