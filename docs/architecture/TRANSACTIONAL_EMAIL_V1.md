...
| `AUDIT_DELIVERED` | `PRODUCER_WIRED` | After staff `complete_audit_and_issue_subscription_credit_v5` or automatic `complete_automatic_audit_generation_v1` success. Email failure never undoes Audit state. |
...
- `AUDIT_DELIVERED` → `audit_delivered:{auditOrderId}`
...
- `supabase/migrations/20260813150000_transactional_email_outbox.sql`
- `supabase/migrations/20260813160000_email_outbox_waiting_configuration_recovery.sql`
...
| `app/api/platform/audit/complete/route.ts` | best-effort `AUDIT_DELIVERED` after completion RPC |
| `apps/mission-worker` | terminal mission notify, automatic Audit delivered/needs-review, **independent email outbox poll** |
