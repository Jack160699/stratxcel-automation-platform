...
/** Reuses the exact same tenant-scoped /api/platform/missions route as app/admin/(shell)/platform/missions/page.tsx — same data, client-facing chrome. */
export default function ClientMissionsPage() {
  ...
  const columns: DataTableColumn<Mission>[] = [
    ...
    { key: "cost", header: "Est. cost", render: (m) => (m.estimated_cost_cents != null ? `₹${(m.estimated_cost_cents / 100).toFixed(2)}` : "—") },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-3">
        ...
        {tenantId && missions?.length === 0 && <EmptyState title="No missions yet." subtitle="Start one from Copilot, or create one above." />}
