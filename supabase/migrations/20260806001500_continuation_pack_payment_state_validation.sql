-- Additive migration: Continuation-pack payment state validation for reconcile_and_fulfill_razorpay_payment_v4
-- Ensures continuation_packs status constraint includes terminal states ('cancelled', 'expired').
-- Restricts continuation pack payments to payable state ('pending').
-- Rejects applied, paid, refunded, cancelled, expired continuation packs with reason = 'continuation_pack_state_not_payable'.

-- 1. Ensure continuation_packs status constraint allows terminal states
...
alter table continuation_packs add constraint continuation_packs_status_check
  check (status in ('pending', 'paid', 'applied', 'refunded', 'cancelled', 'expired'));
