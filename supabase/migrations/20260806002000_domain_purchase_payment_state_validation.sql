-- Additive migration: Domain purchase payment state validation for reconcile_and_fulfill_razorpay_payment_v4
-- Ensures domains status constraint includes post-payment and terminal states.
-- Restricts domain_purchase payments to payable state ('pending_payment').
-- Rejects paid, paid_pending_registration, registered, active, refunded, cancelled, expired with reason = 'domain_purchase_state_not_payable'.

-- 1. Ensure domains status constraint allows post-payment and terminal states
...
alter table domains add constraint domains_status_check
  check (status in ('pending_payment', 'paid_pending_registration', 'registering', 'registered', 'paid', 'active', 'failed', 'expired', 'transferred_out', 'refunded', 'cancelled'));
