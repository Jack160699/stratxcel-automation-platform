...
function run() {
  ...
  // --- 5. /products redirects to /modules; /solutions is canonical;
  // /use-cases redirects for bookmarks ------------------------------------
  ...
  assert.ok(/<PublicHeader\s*\/>/.test(modulesSource), "app/modules must render the product discovery experience");
