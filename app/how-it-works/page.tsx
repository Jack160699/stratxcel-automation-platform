...
const AUDIT_FAQ = PRICING_OBJECTIONS.filter((i) =>
  ["audit_flow", "audit_subscription", "staff_activation"].includes(i.id),
);
  ...
  return (
    <PublicPageShell>
      ...
      <ConversionFaqSection
        ...
        items={AUDIT_FAQ}
        className="border-t border-sx-border bg-sx-surface-1 py-14"
      />

      <section className="mx-auto max-w-6xl px-4 py-16 text-center sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
          <TrackedCtaLink
            href={PUBLIC_CTAS.audit.href}
            event="start_audit"
            surface="how_it_works_audit"
            plan="audit"
