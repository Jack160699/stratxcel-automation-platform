-- Additive migration: Audit-credit price revalidation for reconcile_and_fulfill_razorpay_payment_v4
-- Dynamically checks valid audit credit & plan base price to compute expected price.
-- Rejects payment if captured amount differs from expected price BEFORE payment_orders creation/update and BEFORE subscription activation.
...
begin
  ...
  -- ============================================================
  -- 5. PRE-MUTATION VALIDATIONS (Product resolution, Plan Tier & Audit-Credit Price Check)
  ...
  if v_purpose = 'wallet_topup' then
    ...
    if v_sub.audit_order_id is not null then
      ...
      if found then
        -- Strict tenant validation: audit tenant MUST match subscription tenant AND payment link tenant
        if v_audit.tenant_id = v_sub.tenant_id and v_audit.tenant_id = v_link.tenant_id then
  ...
  -- ============================================================
  -- 6. ATOMIC MUTATIONS (Executed only after product, purpose & price validation passed)
  -- ============================================================
