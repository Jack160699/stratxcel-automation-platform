...
const INTERNAL_STATE_LINE = /^.*(?:STORED_UNREADABLE|UPLOADED|EXTRACTED|processingStatus|processing_status|mediaAssetId|attachmentId|storage_path).*$/gim;
const MALFORMED_STRUCTURED_LINE = /^\s*(?:[\[{].*[}\]]|.*(?:UNREADABLE|UPLOADED|EXTRACTED)["'}\]}:,]*)\s*$/gim;
...
export function sanitizeUserFacingText(value: string): string {
  const cleaned = value
    ...
    .replace(INTERNAL_STATE_LINE, "")
    .replace(MALFORMED_STRUCTURED_LINE, "")
    .replace(INTERNAL_ID_LINE, "")
    .replace(UUID, "[internal reference hidden]")
    .replace(INTERNAL_TOOL_NAME, "internal workflow step")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
  return cleaned || (value.trim() ? "We couldn’t display this message safely." : "");
}
