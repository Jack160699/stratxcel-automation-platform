...
const INJECTION_PATTERNS: RegExp[] = [
  ...
  /send\s+(?:the\s+)?(?:api\s*key|secrets?|credentials?)/i,
  /execute\s+(?:this\s+)?(?:command|shell|code)/i,
  /invoke\s+(?:workforce|capability|tool)/i,
  /publish\s+(?:to\s+)?(?:social|whatsapp|website)/i,
  /publish\s+this\s+content/i,
  /change\s+(?:the\s+)?(?:system|config|subscription|entitlement)/i,
  /deploy\s+(?:the\s+)?website/i,
  /delete\s+(?:the\s+)?website/i,
  /promote\s+(?:our\s+)?competitor/i,
  /call\s+(?:this\s+)?api/i,
