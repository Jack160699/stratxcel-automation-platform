...
export function decideFromScore(
  policy: QualityPolicy,
  scores: QualityScore[],
  opts: { missingEvidence?: boolean; brandGateFailed?: boolean } = {},
): QualityDecision {
  ...
  if (factuality < 60 && originality > 90) return "REVISE";
...
export function critiqueCandidate(input: CritiqueInput): QualityCritiqueResult {
  ...
  const brandGateFailed = (input.scoreOverrides?.brand_fit ?? 80) < (input.policy.thresholds.minimumByDimension.brand_fit ?? 75);
  ...
  return {
    ...
    weaknesses: decision === "PASS" ? [] : ["Quality threshold not met"],
    requiredChanges: decision === "PASS" ? [] : ["Address scored weaknesses before finalization"],
