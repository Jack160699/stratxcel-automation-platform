...
assert.match(shareRoute, /token_hash/);
...
const emailRoute = readFileSync(new URL("../../../app/api/platform/audit/report/email/route.ts", import.meta.url), "utf8");
assert.match(emailRoute, /audit_delivered:\$\{ctx\.order\.id\}/);
assert.match(emailRoute, /audit_delivered_resend:/);
assert.match(emailRoute, /Could not queue the report email/);
...
assert.match(whatsappRoute, /no_permitted_destination/);
