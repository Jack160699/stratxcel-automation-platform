// Verifies the mission state machine matches the master brief's diagram
// exactly, including the documented BLOCKED/CANCELLED interpretation.
// Run with: node --experimental-strip-types lib/missions/__tests__/state-machine.test.ts

import assert from "node:assert/strict";
import { canTransition, assertTransition, isTerminalState, InvalidMissionTransitionError } from "../state-machine.ts";

function run() {
  // Happy path through the whole diagram
  ...
  // BLOCKED can resume back into the queue
  assert.equal(canTransition("BLOCKED", "QUEUED"), true);

  // assertTransition throws a typed error on an invalid edge
  ...
  console.log("state-machine.test.ts: ALL PASS");
}
