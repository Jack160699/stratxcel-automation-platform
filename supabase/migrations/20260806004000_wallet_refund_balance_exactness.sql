-- Additive migration: process_refund_atomic_v7
-- Enforces exact wallet balance validation for wallet_topup refunds.
-- Requires tenant wallet_accounts row to exist and balance_cents >= p_actual_refund_amount_cents.
-- If missing or insufficient, sets status = 'MANUAL_REVIEW', creates zero ledger entries / balance mutations, and returns success = false.
-- Exact subtraction (no greatest/clamping) and exact ledger insertion.
-- Revokes execution of process_refund_atomic_v6 and restricts v7 execution strictly to service_role.

create or replace function public.process_refund_atomic_v7(
  p_refund_id uuid,
...
begin
  ...
  if v_purpose = 'wallet_topup' then
    ...
    -- Exact subtraction (NO clamping/greatest)
...
-- Revoke execution of process_refund_atomic_v6 from all roles
revoke execute on function public.process_refund_atomic_v6(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated, service_role;

-- Grant execution of process_refund_atomic_v7 only to service_role
revoke execute on function public.process_refund_atomic_v7(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated;
grant execute on function public.process_refund_atomic_v7(uuid, uuid, text, text, bigint, text, text) to service_role;
