-- PR #24 recurring AutoPay fixes. Additive. Does NOT touch payment-v4.

alter table public.subscriptions
  add column if not exists audit_credit_deferred_cents bigint not null default 0,
  add column if not exists provider_offer_id text;

drop function if exists public.apply_razorpay_subscription_lifecycle_event(text, text, text, timestamptz, text);
...
begin
  ...
  update subscriptions set
    ...
    current_period_start = case when v_is_updated then coalesce(p_current_period_start, current_period_start) else current_period_start end,
    current_period_end = case when v_is_updated then coalesce(p_current_period_end, current_period_end) else current_period_end end,
    pending_plan_tier = case
      when v_is_updated and p_pending_plan_tier in ('starter', 'growth', 'business') then p_pending_plan_tier
...
revoke execute on function public.apply_razorpay_subscription_lifecycle_event(text, text, text, timestamptz, text, text, timestamptz, timestamptz, text, text) from public, anon, authenticated;
grant execute on function public.apply_razorpay_subscription_lifecycle_event(text, text, text, timestamptz, text, text, timestamptz, timestamptz, text, text) to service_role;

drop function if exists public.reconcile_and_fulfill_razorpay_subscription_charge(text, text, text, text, bigint, text, text, boolean, text);
...
declare
  ...
  v_is_first_charge boolean := false; v_period_start timestamptz; v_period_end timestamptz;
...
begin
  ...
  if coalesce(trim(v_sub.provider_plan_id), '') <> '' and trim(p_provider_plan_id) <> trim(v_sub.provider_plan_id)
    ...
    return jsonb_build_object('fulfilled', false, 'reason', 'provider_plan_mismatch', 'expected_provider_plan_id', v_sub.provider_plan_id, 'received_provider_plan_id', p_provider_plan_id);
  end if;
  ...
  if p_actual_amount_cents <> v_expected_cents then
    ...
    return jsonb_build_object('fulfilled', false, 'reason', 'amount_mismatch', 'expected_amount_cents', v_expected_cents, 'received_amount_cents', p_actual_amount_cents);
  end if;
  ...
  if v_order_exists then
    update payment_orders set state = 'CAPTURED', provider_order_id = coalesce(p_provider_order_id, provider_order_id),
      ...
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('provider_subscription_id', p_provider_subscription_id, 'provider_plan_id', p_provider_plan_id, 'provider_event_id', p_provider_event_id, 'event_type', p_event_type),
      updated_at = now()
    ...
    begin
      ...
      values (v_sub.tenant_id, 'razorpay', p_provider_payment_id, p_provider_order_id, p_actual_amount_cents, p_actual_currency, 'CAPTURED', 'subscription_payment', v_mode, 'razorpay_subscription', v_sub.id::text,
        jsonb_build_object('provider_subscription_id', p_provider_subscription_id, 'provider_plan_id', p_provider_plan_id, 'provider_event_id', p_provider_event_id, 'event_type', p_event_type))
  ...
  update subscriptions set status = 'active', plan_tier = v_effective_plan,
    ...
    provider_plan_id = coalesce(nullif(trim(p_provider_plan_id), ''), provider_plan_id),
    activated_at = coalesce(activated_at, now()), current_period_start = v_period_start, current_period_end = v_period_end,
    ...
    last_provider_payment_id = p_provider_payment_id, next_charge_at = v_period_end,
  ...
  return jsonb_build_object(
    ...
    'plan_tier', v_effective_plan, 'mode', v_mode, 'audit_credit_deferred_applied', v_deferred_valid
  );
end;
$$;

revoke execute on function public.reconcile_and_fulfill_razorpay_subscription_charge(text, text, text, text, bigint, text, text, boolean, text, text, text, text, text) from public, anon, authenticated;
grant execute on function public.reconcile_and_fulfill_razorpay_subscription_charge(text, text, text, text, bigint, text, text, boolean, text, text, text, text, text) to service_role;
