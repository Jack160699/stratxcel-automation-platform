-- Additive migration: process_refund_atomic_v9
-- Enforces subscription refund reversal exactness.
-- Validates exact payment order to payment link to subscription relationship and tenant isolation.
-- Restricts automatically refundable subscription state strictly to 'active'.
-- Requires base unit removal to leave current_usage <= bonus_limit_amount on every linked usage_entitlements row.
-- On success: sets base_limit_amount = 0, limit_amount = bonus_limit_amount, inserts negative entitlement ledger entries (-old_base_val), sets subscription status = 'refunded', and marks refund PROCESSED.
-- On rejection: returns MANUAL_REVIEW with zero mutations to subscription, entitlements, or ledger.
-- Revokes execution of process_refund_atomic_v8 and restricts v9 execution strictly to service_role.

create or replace function public.process_refund_atomic_v9(
  p_refund_id uuid,
...
begin
  ...
  if v_purpose = 'wallet_topup' then
    ...
    if v_order.reference_id is null or trim(v_order.reference_id) = '' then
    ...
    select * into v_link from payment_links where id::text = v_order.reference_id for update;
    if not found then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'subscription_refund_relationship_mismatch' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'subscription_refund_relationship_mismatch', 'status', 'MANUAL_REVIEW');
    end if;

    if v_link.tenant_id <> v_order.tenant_id or v_link.payment_purpose <> 'subscription_payment' then
      update payment_refunds set status = 'MANUAL_REVIEW', reason = 'subscription_refund_relationship_mismatch' where id = p_refund_id;
      return jsonb_build_object('success', false, 'reason', 'subscription_refund_relationship_mismatch', 'status', 'MANUAL_REVIEW');
    end if;
    ...
    -- Lock and Check Entitlements (Safe Base Unit Removal)
    perform 1 from usage_entitlements
    where (subscription_id = v_sub.id or tenant_id = v_sub.tenant_id)
    ...
    -- Execute Entitlement Base Unit Reversal
    for v_ent_rec in (
      select * from usage_entitlements
      where (subscription_id = v_sub.id or tenant_id = v_sub.tenant_id) and base_limit_amount > 0
      for update
    ) loop
      if v_ent_rec.tenant_id <> v_sub.tenant_id then
        update payment_refunds set status = 'MANUAL_REVIEW', reason = 'consumed_or_unverifiable_subscription_units_manual_review' where id = p_refund_id;
        return jsonb_build_object('success', false, 'reason', 'consumed_or_unverifiable_subscription_units_manual_review', 'status', 'MANUAL_REVIEW');
      end if;
...
-- Revoke execution of process_refund_atomic_v8 from all roles
revoke execute on function public.process_refund_atomic_v8(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated, service_role;

-- Grant execution of process_refund_atomic_v9 only to service_role
revoke execute on function public.process_refund_atomic_v9(uuid, uuid, text, text, bigint, text, text) from public, anon, authenticated;
grant execute on function public.process_refund_atomic_v9(uuid, uuid, text, text, bigint, text, text) to service_role;
