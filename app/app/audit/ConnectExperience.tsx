    ...
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      {step === "connect" && (
        <section className="mt-6 space-y-4">
          ...
          <label className="block text-sm font-medium text-sx-text">Website
            <input
              value={website}
          ...
          <div>
            <label className="text-sm font-medium text-sx-text">Add business channel
              <select value={addType} onChange={(event) => setAddType(event.target.value as AuditChannelType)} className="mt-1 ml-2 rounded-sx-sm border border-sx-border-strong bg-sx-surface-1 px-2 py-1 text-sm">
                {AUDIT_CHANNEL_TYPES.map((type) => <option key={type} value={type}>{AUDIT_CHANNEL_LABELS[type]}</option>)}
              </select>
            </label>
          ...
          <ul className="space-y-3">
            {channels.map((channel) => (
              <li key={channel.type} className="rounded-sx-sm border border-sx-border p-3">
                <div className="flex items-center justify-between text-sm font-medium">{AUDIT_CHANNEL_LABELS[channel.type]}
      ...
      {step === "questions" && (
        <section className="mt-6 space-y-4">
          ...
          <button disabled={busy} onClick={() => void call("save_answers", { answers }).then((ok) => ok && call("finalize"))} className="min-h-11 rounded-sx-sm bg-sx-accent px-5 text-sm font-semibold text-sx-accent-on">
