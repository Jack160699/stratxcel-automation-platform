...
export const APP_SIDEBAR_GROUPS: SidebarNavGroup[] = APP_NAV_GROUPS_DATA.map((group) => ({
  label: group.label,
  items: group.items.map((item): SidebarNavItem => ({ ...item, labelHi: item.labelHi, icon: NAV_ICONS[item.key] ?? <DocIcon /> })),
}));
...
export const APP_MOBILE_NAV: SidebarNavItem[] = flattenNavGroups(APP_NAV_GROUPS_DATA)
  ...
  .map((item) => ({ ...item, labelHi: item.labelHi, icon: NAV_ICONS[item.key] ?? <DocIcon /> }));
