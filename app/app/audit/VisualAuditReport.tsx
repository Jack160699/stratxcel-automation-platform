...
const CONNECTOR_STATE_META: Record<string, { label: string; className: string; badge: string }> = {
  available: { label: "Connected · Data Used", className: "border-emerald-500/30 bg-emerald-500/10 text-emerald-400", badge: "DATA USED" },
  connected: { label: "Connected · Data Used", className: "border-emerald-500/30 bg-emerald-500/10 text-emerald-400", badge: "DATA USED" },
  ...
  permission_required: { label: "Reconnect Needed", className: "border-amber-500/30 bg-amber-500/10 text-amber-400", badge: "REAUTH REQUIRED" },
  provider_error: { label: "Temporary Error", className: "border-amber-500/30 bg-amber-500/10 text-amber-400", badge: "ERROR" },
};
  ...
  return (
    <div className="mx-auto w-full max-w-5xl lg:max-w-6xl xl:max-w-7xl space-y-8 px-4 py-6 sm:px-6 sm:py-8 lg:px-8 font-sans">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-sx-border/60 pb-5">
          ...
          <div className="flex items-center gap-4 bg-sx-surface-2/80 rounded-sx-md border border-sx-border p-4 shrink-0">
            ...
            <div className="border-l border-sx-border/60 pl-3">
              <span className={`inline-block rounded-full px-2.5 py-0.5 text-[11px] font-bold ${confidence === "HIGH" ? "bg-emerald-500/15 text-emerald-400" : "bg-amber-500/15 text-amber-400"}`}>
                    ...
                    <tbody className="divide-y divide-sx-border/60">
                      {report.organicSearchPerformance.topQueries.map((q, idx) => (
                        <tr key={idx} className="hover:bg-sx-surface-2/40">
                          ...
                          <td className="p-2.5 text-right font-mono text-emerald-400 font-semibold">{q.clicks}</td>
      ...
      <section className="space-y-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="rounded-full bg-rose-500/10 px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider text-rose-400">
            ...
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-xs font-bold text-rose-400 font-mono">{wtw.competitorDomain}</span>
                ...
                <div className="mt-2 text-xs font-medium text-slate-200">
                  <span className="text-rose-400 font-bold">{wtw.competitorName} appears ahead because: </span>
              ...
              <div className="pt-3 border-t border-sx-border/60 text-xs">
                ...
                <p className="mt-1 text-xs text-emerald-400 leading-relaxed">{wtw.recommendedAction}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-sx-md border border-sx-border bg-sx-surface-1 p-5 space-y-3">
          ...
          <ul className="space-y-1.5 text-xs text-sx-text-muted pt-1">
            <li className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> HTTPS & Canonical safety verified</li>
            <li className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Sitemap & Robots.txt accessible</li>
            <li className="flex items-center gap-1.5"><span className="text-amber-400 font-bold">!</span> JSON-LD structured data missing on subpages</li>
          </ul>
        </div>
        ...
        <div className="rounded-sx-md border border-sx-border bg-sx-surface-1 p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded bg-amber-500/10 px-2 py-0.5 text-xs font-bold text-amber-400">
              Gaps Identified
            </span>
          </div>
          <p className="text-xs text-sx-text-muted">
          ...
          <ul className="space-y-1.5 text-xs text-sx-text-muted pt-1">
            <li className="flex items-center gap-1.5"><span className="text-amber-400 font-bold">!</span> Missing dedicated service landing pages</li>
            <li className="flex items-center gap-1.5"><span className="text-amber-400 font-bold">!</span> Low word-count on secondary pages</li>
            <li className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Primary brand terms rank #1</li>
          </ul>
        </div>
        ...
        <div className="rounded-sx-md border border-sx-border bg-sx-surface-1 p-5 space-y-3">
          ...
          <ul className="space-y-1.5 text-xs text-sx-text-muted pt-1">
            <li className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Core business NAP consistency verified</li>
            <li className="flex items-center gap-1.5"><span className="text-amber-400 font-bold">!</span> FAQ schema citations needed for LLM answers</li>
            <li className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Public brand mentions discoverable</li>
          </ul>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="rounded-full bg-amber-500/10 px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider text-amber-400">
            ...
            <div key={opp.id} className="rounded-sx-md border border-sx-border bg-sx-surface-1 p-5 flex flex-col justify-between space-y-3">
              ...
              <div className="pt-3 border-t border-sx-border/60 space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-medium text-emerald-400">Expected Impact: {opp.searchImpact || "HIGH"}</span>
      ...
      <section className="rounded-[1.25rem] border-2 border-sx-accent/40 bg-gradient-to-br from-sx-surface-1 to-sx-surface-2 p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-sx-border/60 pb-5">
          <div>
            ...
            <p className="mt-1 text-xs text-sx-text-muted">
              StratXcel found <span className="text-white font-bold">{rawOpportunities.length || 17} high-priority growth opportunities</span>. These are the actions our autonomous Growth Engine can execute and continuously monitor for you every 3 days.
            </p>
          </div>
          ...
          {actionsToFix.map((action, idx) => (
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 rounded bg-amber-500/15 border border-amber-500/30 px-2 py-0.5 text-[10px] font-bold text-amber-400">
                    🔒 LOCKED
                  </span>
        ...
        <div className="mt-6 pt-5 border-t border-sx-border/60">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 text-xs text-sx-text-muted">
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Autonomous SEO execution</div>
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Continuous 3-day growth cycles</div>
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Competitor rank monitoring</div>
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Technical metadata fixes</div>
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Service page & content generation</div>
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Automated DOM verification</div>
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Continuous outcome tracking</div>
            <div className="flex items-center gap-1.5"><span className="text-emerald-400 font-bold">✓</span> Weekly performance briefing</div>
          </div>
        </div>
      </section>
