...
const OUT_DIR = path.join(process.cwd(), ".screenshots-canonical-6step-onboarding");
...
async function run() {
  console.log("================================================================================");
  console.log("STARTING CANONICAL 6-STEP STRATXCEL ONBOARDING E2E VALIDATION");
  ...
  console.log(">>> Step 1: Account (Google Identity + Website & Google Maps inputs)");
  {
    ...
    // ---------------------------------------------------------------------------
    // 5. STEP 5: BRAND (AI Pre-filled Guidelines)
    // ---------------------------------------------------------------------------
    console.log("\n>>> Step 5: Brand (AI Pre-filled Guidelines)");
    const step5Text = await page.textContent("body");
    assert.ok(step5Text?.includes("Help StratXcel understand how your business should sound"), "Must show Step 5 headline");
    assert.ok(step5Text?.includes("Pre-filled from your business information"), "Must show pre-filled badge");
    assert.ok(step5Text?.includes("Short Business Description"), "Must show description field");
    assert.ok(step5Text?.includes("Target Audience"), "Must show audience field");
    assert.ok(step5Text?.includes("Tone of Voice"), "Must show tone field");

    await page.screenshot({ path: path.join(OUT_DIR, "step5-brand-guidelines.png") });
    console.log("  ✓ Step 5: AI pre-filled brand guidelines verified.");
    ...
    // ---------------------------------------------------------------------------
    // 6. STEP 6: REVIEW (Final Verification Summary)
    // ---------------------------------------------------------------------------
    console.log("\n>>> Step 6: Review (Summary & Audit CTA)");
    const step6Text = await page.textContent("body");
    assert.ok(step6Text?.includes("Everything looks good?"), "Must show Step 6 headline");
    assert.ok(step6Text?.includes("Account & Channels"), "Must show Account section");
    assert.ok(step6Text?.includes("Business Identity"), "Must show Business section");
    assert.ok(step6Text?.includes("Focus Priorities"), "Must show Focus Priorities section");
    assert.ok(step6Text?.includes("Brand Signals"), "Must show Brand Signals section");
    assert.ok(step6Text?.includes("Start My Free Business Audit →"), "Must show canonical Audit CTA button");
    assert.equal(/Workspace Slug/i.test(step6Text || ""), false, "Workspace slug must NOT be in Review summary");
    assert.equal(/₹9,999|pricing|package/i.test(step6Text || ""), false, "Pricing/packages must NOT be in Review summary");

    await page.screenshot({ path: path.join(OUT_DIR, "step6-review-summary.png") });
    console.log("  ✓ Step 6: Final Review summary and Free Business Audit CTA verified.");

    await context.close();
  }

  // ---------------------------------------------------------------------------
  // 7. MULTI-VIEWPORT RESPONSIVE LAYOUT VERIFICATION (7 VIEWPORTS)
  // ---------------------------------------------------------------------------
  console.log("\n>>> Phase 7: 7-Viewport Responsive Visual Check");
  ...
  await browser.close();
  console.log("\n================================================================================");
  console.log("ALL CANONICAL 6-STEP ONBOARDING E2E TESTS PASSED SUCCESSFULLY");
  console.log("================================================================================");
}
